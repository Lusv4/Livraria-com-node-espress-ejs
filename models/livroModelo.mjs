...
class LivroModelo {
    ...
    static async criarLivro({ titulo, genero, autor, editora, ISBN, quantidade, dataPublicacao }) {
        ...
        const resultado = await requisicao.query(`
            INSERT INTO Livros (titulo,genero,autor,editora,ISBN,quantidade,dataPublicacao)
            OUTPUT INSERTED.uid
            VALUES(@titulo,@genero,@autor,@editora,@ISBN,@quantidade,@dataPublicacao)
            `)
        const uid = resultado.recordset[0].uid // Retorna o ID unico
        return {
            uid, titulo, genero, autor, editora, ISBN: Number(ISBN),
            quantidade: Number(quantidade), dataPublicacao: new Date(dataPublicacao)
        }
    }
