...
class PedidoModelo{
    ...
    static async criarPedido({funcionarioID, usuarioID, livrosUIDS, datasDevolucao}){
        const livrosSelecionados = Array.isArray(livrosUIDS) ? livrosUIDS : livrosUIDS ? [livrosUIDS] :[] //se o vetor livrosUIDS for diferente de vazio vai entregar o vetor livrosUIDS, se o vetor for diferente do vetor livrosUIDS retorna um vetor vazio
        ...
        try{
            ...
            for (let contador = 0; contador < livrosSelecionados.length; contador++){
                ...
                await itemPedido.query('INSERT INTO Item_Pedido (PedidoID, LivroID, DataDevolucao) VALUES (@idPedido, @livroID, @dataDevolucao)')
            }
